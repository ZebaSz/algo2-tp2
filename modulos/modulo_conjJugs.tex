...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		\NoCaptionOfAlgo
		\caption{\algoritmo{iAgregar}{\Inout{c}{eJugs}, \In{j}{jugador}, \In{n}{nat}}{}}
		\If{Pertenece?(p, j)}{
			Borrar(p, j)\OdeLinea{log(\#j)}
		}
		p.raiz $\leftarrow$ Insertar(j, n, p.raiz)\OdeLinea{log(\#j)}
	\end{algorithm}
	...
	\begin{algorithm}[H]
		...
		puntero(Nodo): nodo $\leftarrow$ Buscar(j, p.raiz)\OdeLinea{log(\#j)}
	...
	\begin{algorithm}[H]
		\NoCaptionOfAlgo
		\caption{\algoritmo{Insertar}{\In{j}{jugador}, \In{n}{nat}, \Inout{nodo}{puntero(Nodo)}}{puntero(Nodo)}}
		\Pre{\texttt{true}}
		...
		\eIf{nodo = NULL}{
			...
			\eIf{j $<$ n.id}{
				(*nodo).hijoIzq $\leftarrow$ Insertar(j, n, (*nodo).hijoIzq)\OdeLinea{h}
			}{
				(*nodo).hijoDer $\leftarrow$ Insertar(j, n, (*nodo).hijoDer)\OdeLinea{h}
			}

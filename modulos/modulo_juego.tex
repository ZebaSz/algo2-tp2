...
\begin{Interfaz}
	...
	\textbf{servicios usados}: \tadNombre{Coordenada}, \tadNombre{Mapa}, \tadNombre{Vector($\alpha$)}, \tadNombre{DiccionarioString($\alpha$)}, \tadNombre{ColaPrioridad($\alpha$)}, \tadNombre{Conjunto Lineal($\alpha$)}, \tadNombre{ConjuntoOrd($\alpha$)}, \tadNombre{Tupla con Orden($\alpha$, $\beta$)}
	...
	\InterfazFuncion{EntrenadoresPosibles}{\In{j}{juego}, \In{es}{conj(jugador)} \In{c}{coor}}{conj(jugador)}
...
\begin{Representacion}
	...
	\begin{Estructura}{Juego}[pokego]
		...
		\begin{Tupla}[infoPos]
			...
			\tupItem{jugEsperandoCaptura}{colaPrior(tupOrd(jugador, nat))}
			\tupItem{jugsEnPos}{conjOrd(jugador)}
		\end{Tupla}
	...
	\Rep[pokego][e]{
		...
		jugadoresEsperandoCaptura(e, e.posConPokemon) $\land$\\
	...
	\tadOperacion{jugsEsperandoEnPosValido}{pokego, colaPrior(jugador, nat)}{bool}{}

	\tadOperacion{jugadoresEstanCapturando}{pokego}{bool}{}

	\tadOperacion{enCapturaValida}{pokego/p, nat/i}{bool}{i $\leq$ long(e.jugadores) $\land$ e.jugadores[i] $\rightarrow$conectado?}

	\tadOperacion{estaEnLaCola}{tuplaOrd(jugador,nat), colaPrior(tuplaOrd(jugador,nat))}{bool}{}
	...
	\tadAxioma{jugadoresEsperandoCaptura(e, ps)}{
		...
		$\land$ jugadoresEsperandoCaptura(e, sinUno(ps)))
	}
	
	~

	\tadAxioma{jugsEsperandoEnPosValido(e, cola)}{
		vacía?(cola) $\oluego$ (jugsEsperandoEnPosValido(e, desencolar(cola)) $\land$ $\pi_1$(proximo(cola)) $<$ long(e.jugadores) $\land$ e.jugadores[$\pi_1$(proximo(cola))].sanciones $<$ 5 $\yluego$ distEuclidea(e.jugadores[$\pi_1$(proximo(cola))].posicion, crearCoor(i,j)) $\leq$ 4 $\land$ $\pi_2$(proximo(cola)) = (*e.jugadores[$\pi_1$(proximo(cola))]).cantPokemons)
	}

	~

	\tadAxioma{jugadoresEstanCapturando(e)}{
	($\forall i$: nat) ((i $\leq$ long(e.jugadores) $\land$ e.jugadores[i] $\rightarrow$conectado?) $\implies$ enCapturaValida(e, i))
	}

	\tadAxioma{enCapturaValida(e, i)}{
		($\forall c$: coor) ((posExistente(c, e.mapa) $\land$ c $\in$ e.posConPokemons $\land$ \\
		distEuclidea(c, e.jugadores[i]$\rightarrow$posicion) $\leq$ 4) $\implies$ \\
		estaEnLaCola($\langle$ i, e.jugadores[i] $\rightarrow$ cantPokemons $\rangle$, \\
		e.grillaPos[longitud(e.jugadores[i]$\rightarrow$posicion)]$ \\ $[latitud(e.jugadores[i]$\rightarrow$posicion)].jugsEsperandoCaptura))
	}

	\tadAxioma{estaEnLaCola(tupla, cola)}{
	\IF vacía?(cola) THEN
		false
	ELSE
		(próximo(cola) = tupla) $\lor$ enCapturaValida(tupla, desencolar(cola))
	FI}
...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		...
		\While(\OdeBloque{EC \times log(EC)}){HaySiguiente(itCoor)}{
			...
			itConjOrd : it $\leftarrow$ CrearIterador(j.grillaPos[Latitud(d)][Longitud(d)].jugsEnPos)\OdeLinea{1}
			\While(\OdeBloque{EC \times log(EC)}){HayMas(it)}{
				jugador: jug $\leftarrow$ Actual(it)\OdeLinea{1}
				Encolar(j.grillaPos[Latitud(c)][Longitud(c)].jugEsperandoCaptura, CrearTupla(jug, (*j.jugadores[jug]).cantPokemons))\OdeLinea{log(EC)}
	...
	\justifcomp: inversamente a Conectarse, al desconectarse el jugador debe salir del conjunto de jugadores en $c$ y de la cola de espera de captura (de haber un pokemón cerca).
	...
	\begin{algorithm}[H]
		...
		\tcc{Si el jugador debe ser eliminado, borramos sus pokémons; si no, lo movemos}
		...
		\tcc{Para cada posición con pokémon, si el movimiento es lejano se suma al contador y se maneja captura; si el jugador entró a una nueva área de captura, se reinicia el contador}
		\While(\OdeBloque{PS \times |P|}){HaySiguiente?(it)}{
			...
			\eIf{DistEuclidea(c, coorConPk) $>$ 4}{
				...
				\eIf{posPk.contadorCaptura = 10}{
					...
					jugador: captor $\leftarrow$ $\pi_1$(Proximo(posPk.jugEsperandoCaptura))\OdeLinea{1}
					(*j.jugadores[captor]).cantPokemons $\leftarrow$ (*j.jugadores[captor]).cantPokemons + 1\OdeLinea{1}
					\eIf(\OdeBloque{|P|}){Definido((*j.jugadores[captor]).pokemonsCapturados, pk)}{
						nat: nuevaCant $\leftarrow$ Obtener((*j.jugadores[captor]).pokemonsCapturados, pk) + 1\OdeLinea{|P|}
						Definir((*j.jugadores[captor]).pokemonsCapturados, pk, nuevaCant)\OdeLinea{|P|}
					}{
						Definir((*j.jugadores[captor]).pokemonsCapturados, pk, 1)\OdeLinea{|P|}
					}
	...
	\begin{algorithm}[H]
		\NoCaptionOfAlgo
		\caption{\algoritmo{iEntrenadoresPosibles}{\In{j}{pokego}, \In{es}{conj(jugador)}, \In{c}{coor}}{conj(jugador)}}
		res $\leftarrow$ Vacio()\OdeLinea{1}
		itColaPrior(tupOrd(jugador,nat)): entrenadores $\leftarrow$ CrearIt(j.grillaPos[Latitud(c)][Longitud(c)].jugEsperandoCaptura)\OdeLinea{1}
		\While(\OdeBloque{EC \times \#(es)}){HayMas?(entrenadores)}{
			jugador: actual $\leftarrow \pi_1$(Actual(entrenadores))\OdeLinea{1}
			\If(\OdeBloque{\#(es)}){Pertenece?(es,actual)}{
				AgregarRapido(res, actual)\OdeLinea{1}
			}
		}
	\end{algorithm}
	...
	\begin{algorithm}[H]
		...
		\While(\OdeBloque{log(EC)}){HaySiguiente(itCoor)}{
			...
			\If{HayPokémonEnPos(j, siguiente).hayPokemon}{
				Encolar(j.grillaPos[Latitud(siguiente)][Longitud(siguiente)].jugEsperandoCaptura, CrearTupla(e,(*j.jugadores[e]).cantPokemons))\OdeLinea{log(EC)}
			}
			Avanzar(itCoor)\OdeLinea{1}
		}
	\end{algorithm}
	...
	\begin{algorithm}[H]
		...
		\While(\OdeBloque{log(EC)}){HaySiguiente(itCoor)}{
			coor: siguiente $\leftarrow$ Siguiente(itCoor)\OdeLinea{1}
			\If{HayPokémonEnPos(j, siguiente).hayPokemon}{
				Borrar(j.grillaPos[Latitud(siguiente)][Longitud(siguiente)].jugEsperandoCaptura, CrearTupla(e,(*j.jugadores[e]).cantPokemons))\OdeLinea{log(EC)}
			}
			Avanzar(itCoor)\OdeLinea{1}
		}
	\end{algorithm}

...
\begin{Representacion}
	...
	\begin{enumerate}
		...
		\item Para cada una de las claves de jugsEsperandoCaptura, el jugador es un jugador válido y se encuentra en posición de captura del pokemon correspondiente %TODO

		\item Para cada una de las claves de jugsEsperandoCaptura, su significado es la cantidad de pokemons %TODO
	...
	\Rep[pokego][e]{
		...
		pksAlejados(e.posConPokemons) $\land$\\ ($\forall i,j$: nat)(j $<$ alto(e.mapa) $\land$ i $<$ ancho(e.mapa) $\yluego$ e.grillaPos[i][j].hayPokemon $\implies$ ($\forall p$: jugador)(p $\in$ claves(e.grillaPos[i][j].jugEsperandoCaptura) $=$ long(e.jugadores) $>$ p $\yluego$ e.jugadores[p].sanciones $<$ 5 $\yluego$ distEuclidea(e.jugadores[p].posicion, crearCoor(i,j)) $\leq$ 4 \\

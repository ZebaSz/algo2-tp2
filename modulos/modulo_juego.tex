...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		...
		AgregarAColas(j, e)\OdeLinea{log(EC)}
	...
	\begin{algorithm}[H]
		...
		Borrar(j.grillaPos[Latitud(c)][Longitud(c)].jugadores, e)\; %log
		RemoverDeColas(j, e)\OdeLinea{log(EC)}
	\end{algorithm}
	...
	\begin{algorithm}[H]
		...
		\eIf{(*j.jugadores[e]).sanciones = 5}{
			...
			(*j.jugadores[e]).posicion $\leftarrow$ c\OdeLinea{1}
			AgregarAColas(j, e, c)\OdeLinea{1}
			Agregar(j.grillaPos[Latitud(c)][Longitud(c)].jugadores, e)\; %log MISMOS CHABON ENUNA MISMA POSICION
		}
		...
		\While(\OdeBloque{PS \times |P|}){HaySiguiente?(it)}{
			...
			\eIf{DistEuclidea(c, coorConPk) $>$ 4}{
				...
				\eIf{posPk.contadorCaptura = 10}{ 
					...
					AgregarAColas(captor, e)\OdeLinea{1}
				}{
					Avanzar(it)\OdeLinea{1}
				}
			}{

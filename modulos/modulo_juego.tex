		...
		\While(\OdeBloque{PS \times |P|}){HaySiguiente?(it)}{
			...
			\eIf{DistEuclidea(c, coorConPk) $>$ 4}{
				...
				\If{DistEuclidea(posAnterior, coorConPk) $>$ 4}{
					...
					Definir(j.grillaPos[Latitud(coorConPk)][Longitud(coorConPk)].jugEsperandoCaptura, e, (*j.jugadores[e]).cantPokemons)\OdeLinea{log(EC)}
					%TODO agregar jugsEnPos (Y REVISAR LOS DEMAS PROBLEMAS DONDE FALTA)
				}
				Avanzar(it)\OdeLinea{1}
			}
		}

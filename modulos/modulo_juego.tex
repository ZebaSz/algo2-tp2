...
\begin{Representacion}
	...
	\begin{Estructura}{Juego}[pokego]
		...
		\begin{Tupla}[infoJugador]
			...
			\tupItem{capturando?}{bool}
			\tupItem{posPokeCaptura}{coor}
...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		...
		%% AGREGARLO A LOS CONJUNTOS DE ESPERA
	...
	\begin{algorithm}[H]
		...
		\IF{(*j.jugadores[e]).capturando?}{
			coor: c $\leftarrow$ (*j.jugadores[e]).posPokeCaptura\;
			(*j.jugadores[e]).capturando? $\leftarrow$ false\;
			Borrar(j.grillaPos[Latitud(c)][Longitud(c)].jugEsperandoCaptura, e)
		}
	\end{algorithm}

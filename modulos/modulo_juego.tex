...
\begin{Representacion}
	...
	\begin{Estructura}{Juego}[pokego]
		...
		\begin{Tupla}[infoPos]
			...
			\tupItem{jugEsperandoCaptura}{colaPrior(tupOrd)}
	...
	\tadOperacion{jugsEsperandoEnPosValido}{pokego, colaPrior(jugador, nat)}{bool}{} %TODO fijense si les parece correcto lo que hice con esto
	...
	\tadAxioma{enCapturaValida(e, i)}{
		...
		($\langle$ i, e.jugadores[i] $\rightarrow$ cantPokemons $\rangle$ $\in$ e.grillaPos[longitud(e.jugadores[i]$\rightarrow$posicion)][latitud(e.jugadores[i]$\rightarrow$posicion)].jugsEnPos))
	}
	...
	%TODO corregir
		...
		\eIf{(*j.jugadores[e]).sanciones = 5}{
			...
			\While(\OdeBloque{PC \times |P|}){HaySiguiente(pokesABorrar)}{
				...
				% TODO: si borramos cuando nuevaCant = 0, la complejidad explota

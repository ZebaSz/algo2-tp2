...
\begin{Representacion}
	...
	\Rep[pokego][e]{
	($\forall c$: coor)(c $\in$ e.posConPokemons $\implies$ c $\in$ coordenadas(e.mapa)) \\
...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		...
		\For(\OdeBloque{EC \times log(EC)}){$i \leftarrow 0$ \KwTo $2$}{
			\For{$j \leftarrow 0$ \KwTo $2 - i$}{
				coor: d $\leftarrow$ CrearCoor(Latitud(c) + i, Longitud(c) + j)\OdeLinea{1} %TODO: Los mas y los menos
				\If{PosExistente(j.mapa, d)}{

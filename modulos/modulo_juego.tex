...
%<<<<<<< HEAD
		...
		\While{i $<$ 5}{
			nat : j $\leftarrow$ 0\;
			coor: coorConPk $\leftarrow$ CrearCoor(Latitud(c) + i, Longitud(c) + j)\;
			\While{DistEuclidea(c, coorConPk) $\leq$ 4}{
				\If{PosExistente(j.mapa, coorConPk $\land$ j.grillaPos[Latitud(c)+ i][Longitud(c) + j].hayPokemon?)}{ %Si la pos existe y hay pokemon
					it $\leftarrow$ CrearIterador(j.grillaPos[Latitud(c)+ i][Longitud(c) + j].jugadores) %TODO: Los mas y los menos
					\While{HayMas(it)}{
...
% >>>>>>> 9e1a13ef3724e8e3d8220af40c9cc23e35cf39ef
	...
	\begin{algorithm}[H]
		...
		ResetearContadores(j, e)\; % O(1) Te Borre lo tuyo seba
	\end{algorithm}
			...
			\While{i $<$ 3}{
				...
				\If{DistEuclidea(c, CrearCoor(Latitud(c) + i, Longitud(c) + j)) $\leq$ 4}{
					\If{Pertenece(j.posConPokemons, CrearCoor(Latitud(c) + i, Longitud(c) + j)}{
						j.grillaPos[Latitud(c) + i][Longitud(c) + j].contadorCaptura $\leftarrow$ 0\; 
					}
				} 
				\If {DistEuclidea(c, CrearCoor(Latitud(c) + i, Longitud(c) - j)) $\leq$ 4}{
					\If{Pertenece(j.posConPokemons, CrearCoor(Latitud(c) + i, Longitud(c) - j)}{

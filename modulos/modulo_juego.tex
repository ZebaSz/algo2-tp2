...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		...
		\While{i $<$ 5}{
			...
			\While{DistEuclidea(c, CrearCoor(Latitud(c) + i, Longitud(c) + j)) $\leq$ 4}{
				\If{PosExistente(j.mapa, CrearCoor(Latitud(c) + i, Longitud(c) + j))}{
					Borrar(j.grillaPos[i][j].jugEsperandoCaptura, e)\;
				}
				j $\leftarrow$ j + 1\;
			}
			i $\leftarrow$ i + 1\;
	...
	\begin{algorithm}[H]
		...
		\caption{\algoritmo{AgregarAColas}{\Inout{j}{pokego}, \In{e}{jugador}}{}}
			...
			\While{i $<$ 5}{
				...
				\While{DistEuclidea(c, CrearCoor(Latitud(c) + i, Longitud(c) + j)) $\leq$ 4}{
					\If{PosExistente(j.mapa, CrearCoor(Latitud(c) + i, Longitud(c) + j))}{
						Definir(j.grillaPos[i][j].jugEsperandoCaptura, e, (*j.jugadores[e]).cantPokemons)\;
					}
					j $\leftarrow$ j + 1\;
				}
				i $\leftarrow$ i + 1\;
			}
			Definir(j.grillaPos[Latitud(c)][Longitud(c)].jugEsperandoCaptura, (*j.jugadores[e]))
	\end{algorithm}

...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		...
		j.grilla[Latitud(c)][Longitud(c)].contadorCaptura $\leftarrow$ 0\;
		%ARMAR LA COLA DE PRIORIDAD
	...
	\begin{algorithm}[H]
		...
		\eIf{(*j.jugadores[e]).sanciones = 5}{
			...
			\While{HaySiguiente(pokesABorrar)}{ % PC ∗ |P|
				tupla(calve: String, significado: Nat) : sig $\leftarrow$ Siguiente(pokesABorrar)\;

				nat: nuevaCant $\leftarrow$ Obtener(j.pokemons, sig.clave) - sig.significado\; %|P|
				Definir(j.pokemons, pk, nuevaCant)\; %|P|
				j.cantPokemons $\leftarrow$ j.cantPokemons - sig.significado\; 
			}
		}{
			(*j.jugadores[e]).posicion $\leftarrow$ c\;
			AgregarAColas(j, e, c)\; %log EC
			Agregar(j.grillaPos[Latitud(c)][Longitud(c)].jugadores, e)\; %log MISMOS CHABON ENUNA MISMA POSICION
		...
		\While{HayMas?(it)}{ %%Itero la posiciones con pks    PS * |P|
			...
			\If{DistEuclidead(c, coorConPk) $>$ 4}{ %me fijo si es mas lejos que el rango de captura
				...
				\If{posPk.contadorCaptura = 10}{ 
					...
					jugador: captor $\leftarrow$ Menor(posPk.jugEsperandoCaptura)\; %Busca el menor o(1)
					(*j.jugadores[captor]).cantPokemons $\leftarrow$ (*j.jugadores[captor]).cantPokemons + 1\; %Aumenta la catidad de polkemons
					\eIf{Definido((*j.jugadores[captor]).pokemonsCapturados, pk)}{ %Define que pokemon captura en el dic |P|
						nat: nuevaCant $\leftarrow$ Obtener((*j.jugadores[captor]).pokemonsCapturados, pk) + 1\;
						Definir((*j.jugadores[captor]).pokemonsCapturados, pk, nuevaCant)\; %|P|
					}{
						Definir((*j.jugadores[captor]).pokemonsCapturados, pk, 1)\; %|P|
					}

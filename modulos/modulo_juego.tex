...
\begin{Representacion}
	...
	\begin{Estructura}{Juego}[pokego]
		\begin{Tupla}[pokego]
			...
			%TODO punteros en consideraciones
...
\begin{Algoritmos}
		...
		\While(\OdeBloque{PS \times |P|}){HaySiguiente?(it)}{
			...
			\eIf{DistEuclidea(c, coorConPk) $>$ 4}{
				...
				\eIf{posPk.contadorCaptura = 10}{
					...
					jugador: captor $\leftarrow$ Proximo(posPk.jugEsperandoCaptura)\OdeLinea{1}
	...
	\begin{algorithm}[H]
		...
		itColaPrior(jugador): entrenadores $\leftarrow$ CrearIt(j.grillaPos[Latitud(c)][Longitud(c)].jugEsperandoCaptura)\OdeLinea{1}
		\While(\OdeBloque{EC \times \#(es)}){HayMas?(entrenadores)}{
			jugador: actual $\leftarrow$ Siguiente(entrenadores).jugador\OdeLinea{1}

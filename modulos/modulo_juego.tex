...
\begin{Representacion}
	...
	\Rep[pokego][e]{
	($\forall c$: coor)(c $\in$ e.posConPokemons $\implies$ c $\in$ coordenadas(e.mapa)) $\land$ e.cantPokemons == sumaSignificadosPoke(e.pokemons.claves, e.pokemons) $\land$ ($\forall c,d$: coor)(c $\in$ e.posConPokemons $\land$ d $\in$ e.posConPokemons $\implies$ c = d $\lor$ distEuclidea(c,d) $>$ 25) $\land$ ($\forall i$:nat)(i $<$ long(e.jugadores) $\implies$ claves(e.jugadores[i].pokemonsCapturados) $\subseteq$ claves(e.pokemons)) $\land$ ($\forall i$:nat)(i $<$ long(e.jugadores)$\implies$(e.jugadores[i].sanciones == 5 $\implies \neg$e.jugadores[i].conectado?)) $\yluego$ ($\forall i$:nat)(i $<$ long(e.jugadores) $/implies$ e.jugadores[i].conectado $\implies$ e.jugadores[i].posicion $\in$ coordenadas(e.mapa))
	}
...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		...
		% TODO algo sobre el contador de movs de las posiciones cercanas
	\end{algorithm}
	...
	\begin{algorithm}[H]
		\NoCaptionOfAlgo
		\caption{\algoritmo{iPokémons}{\In{j}{pokego}}{itBi(tupla(pokemon, nat))}}
		res $\leftarrow$ CrearIterador(j.pokemons)\;
	\end{algorithm}
	...
	\begin{algorithm}[H]
		...
		\While{i $<$ 5}{
			nat : j $\leftarrow$ 0\;
			\While{DistEuclidea(c, CrearCoor(i,j)) $\leq$ n}{
				coor : d $\leftarrow$ CrearCoor(i,j)\;

...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		...
		\For(\OdeBloque{\#posConPokemon}){$i \leftarrow 0$ \KwTo $2$}{
			\For(\OdeBloque{\#posConPokemon}){$j \leftarrow 0$ \KwTo $2$}{
				coor : ne $\leftarrow$ CrearCoor(Latitud(c) + i, Longitud(c) + j)\OdeLinea{1}
				\If(\OdeBloque{\#posConPokemon}){DistEuclidea(c, ne) $\leq$ 4 \textbf{and} Pertenece(j.posConPokemons, ne}{
					Borrar(j.grillaPos[Latitud(ne)][Longitud(ne)].jugEsperandoCaptura, e)\OdeLinea{1}
				}
				\If{Longitud(c) $>$ j}{
					coor : no $\leftarrow$ CrearCoor(Latitud(c) + i, Longitud(c) - j)\OdeLinea{1}
					\If(\OdeBloque{\#posConPokemon}){DistEuclidea(c, no) $\leq$ 4 \textbf{and} Pertenece(j.posConPokemons, no}{
						Borrar(j.grillaPos[Latitud(no)][Longitud(no)].jugEsperandoCaptura, e)\OdeLinea{1}
					}
				}

				\If{Latitud(c) $>$ i}{
					coor : se $\leftarrow$ CrearCoor(Latitud(c) - i, Longitud(c) + j)\OdeLinea{1}
					\If(\OdeBloque{\#posConPokemon}){DistEuclidea(c, se) $\leq$ 4 \textbf{and} Pertenece(j.posConPokemons, se}{
						Borrar(j.grillaPos[Latitud(se)][Longitud(se)].jugEsperandoCaptura, e)\OdeLinea{1}
					}
				}

				\If{Latitud(c) $>$ i \textbf{and} Longitud(c) $>$ j}{
					coor : so $\leftarrow$ CrearCoor(Latitud(c) - i, Longitud(c) - j)\OdeLinea{1}
					\If(\OdeBloque{\#posConPokemon}){DistEuclidea(c, so) $\leq$ 4 \textbf{and} Pertenece(j.posConPokemons, so}{
						Borrar(j.grillaPos[Latitud(so)][Longitud(so)].jugEsperandoCaptura, e)\OdeLinea{1}
					}
				}
			}
		}
	...
	\justifcomp: solo se retorna el conjunto por referencia.
	...
	\begin{algorithm}[H]
		...
		\While{i $<$ n + 1}{
			...
			\While{j $<$ n + 1}{
				nat : j $\leftarrow$ 0\;
				\If{DistEuclidea(c, CrearCoor(Latitud(c) + i, Longitud(c) + j)) $\leq$ n}{
					\If{Pertenece(j.posConPokemons, CrearCoor(Latitud(c) + i, Longitud(c) + j)}{
						res $\leftarrow$ true\;
						\textbf{return}\;
					}

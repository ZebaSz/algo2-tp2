...
\begin{Representacion}
	...
	\Rep[eCola][e]{menor(CrearIt(e.conjElem), Actual(e.menor))}

	\TituloDis{Operaciones auxiliares}

	\tadOperacion{menor}{itConjOrd($\alpha$), $\alpha$}{bool}{}

 	~

	\tadAxioma{menor(itConj, elemento)}{
		HaySiguiente?(itConj) $\impluego$ ((Actual(itConj) $>$ elemento $\lor$ Actual(itConj) = elemento) $\land$ menor(Avanzar(itConj), elemento))
	}

...
\begin{Algoritmos}
	...
	\begin{algorithm}[H]
		...
		\If{res.raizJugadores $\oluego$ n $<$ p.menor $\lor$ (n = p.menor $\land$ j $<$ p.menorId}{
			p.menor $\leftarrow$ n\;
			p.menorId $\leftarrow$ j\;
		}
	...
	\begin{algorithm}[H]
		...
		\If{j = p.menorId $\land$ p.raizCantidad $\neq$ NULL}{
			puntero(Nodo): minimo $\leftarrow$ BuscarMinimo(p.raizCantidad)\;
			p.menor $\leftarrow$ (*minimo).cantPokemons\;
			p.menorId $\leftarrow$ (*minimo).id\;	
		}
	\end{algorithm}
	...
	\begin{algorithm}[H]
		...
		\uElseIf{j $<$ (*nodo).id}{
			res $\leftarrow$ BuscarJ(j, (*nodo).hijoIzq)\OdeLinea{log(\#j)}
		}
		\Else{
			res $\leftarrow$ BuscarJ(j, (*nodo).hijoDer)\OdeLinea{log(\#j)}
		}
	\end{algorithm}
	...
	\begin{algorithm}[H]
		...
		\eIf{nodo = NULL}{
			...
			\eIf{j $<$ n.id}{
				(*nodo).hijoIzq $\leftarrow$ InsertarJ(j, n, (*nodo).hijoIzq)\OdeLinea{log(\#j)}
			}{
				(*nodo).hijoDer $\leftarrow$ InsertarJ(j, n, (*nodo).hijoDer)\OdeLinea{log(\#j)}
			}
	...
	\begin{algorithm}[H]
		...
		\uElseIf{j $<$ (*nodo).id}{
			(*nodo).hijoIzq $\leftarrow$ RemoverJ(j, (*nodo).hijoIzq)\OdeLinea{log(\#j)}
			res $\leftarrow$ Balancear(nodo)\OdeLinea{1}
		}
		\uElseIf{j $>$ (*nodo).id}{
			(*nodo).hijoDer $\leftarrow$ RemoverJ(j, (*nodo).hijoDer)\OdeLinea{log(\#j)}
			res $\leftarrow$ Balancear(nodo)\OdeLinea{1}
		}
		\Else{
